...
def process_log_line(line: str, sample_start: float = 0.0, sample_end: float = 1.0, ep_config: dict = None) -> Optional[ReplayJob]:
    ...
    try:
        ...
        if tokens_count < 3700 or tokens_count > 4000:
            return None
...
def main(args, sample_start, sample_end):
    ...
    try:
        ...
        if args.small_set_limit and args.small_set_limit > 0:
            logger.info(f"Using small set limit: {args.small_set_limit}")
            # Start the small set log reader thread
            reader_thread = threading.Thread(target=log_reader_smallset_thread, args=(args.input, args.small_set_limit, sample_start, sample_end, ep_config))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--preload-time', '-p', type=int, default=10,
    ...
    parser.add_argument("--small-set-limit", type=int, default=None)
